...
bool sim_sched::one_instantiated(false);
...
sim_sched::instantiate_one_node(void)
{
//   assert(!all_instantiated);
   assert(!one_instantiated);
...
sim_sched::handle_create_one_node(deterministic_timestamp ts, size_t n, node::node_id start_id)
{
#ifdef DEBUG
...
//               instantiate_all_nodes();
         	instantiate_one_node();
               //all_instantiated = true;
		       one_instantiated = true;
            }
         }
...
switch(reply[1]) {
//case CREATE_N_NODES:
case CREATE_ONE_NODE:
            //handle_create_n_nodes((deterministic_timestamp)reply[2],
			handle_create_one_node((deterministic_timestamp)reply[2],
                  (size_t)reply[4],
