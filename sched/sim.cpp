...
// not checking for threaded mode 
 // assert(thread_mode);
...
sim_sched::handle_create_n_nodes(deterministic_timestamp ts, size_t n, node::node_id start_id)
{
   ...
   for(message_type i(0); i != n; ++i) {
      ...
      //if(!thread_mode || all_instantiated) { // removing threaded mode
         if(all_instantiated) {
...
//         if(thread_mode && !all_instantiated) {  //removing thread mode
if(!all_instantiated) {  
...
//         cout << "Not available" << endl;
		...
		switch(reply[1]) {
			/* case USE_THREADS:  //removing thread mode
...
{
	//if(thread_mode) { //removing thread mode
