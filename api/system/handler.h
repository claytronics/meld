...
//              (ie. 1 - success, no reply sent; 2 - success, reply sent)
 ...
 #define NUM_HANDLERS	32		// could be larger, but using 32-bit var for flagging
 typedef byte 		Event;			// ensure events are unsigned and variable length can store NUM_HANDLERS
 typedef uint64_t 	HandleMask;	// must exceed or match NUM_HANDLERS
