...
get_node_val(pcounter& m)
{
   ...
   assert(ret <= vm::All->DATABASE->max_id());
...
{
   tuple *tuple(vm::All->PROGRAM->new_tuple(alloc_predicate(pc)));
...
execute_send_self(tuple *tuple, state& state)
{
   ...
   if(tuple->is_action()) {
      if(state.count > 0)
         vm::All->MACHINE->run_action(state.sched,
               state.node, tuple);
      else
         delete tuple;
      return;
...
float_val get_op_function<float_val>(const instr_val& val, pcounter& m, state& state)
{
   if(val_is_float(val)) {
      ...
      return tuple->get_float(field);
		...
		float_val val(vm::All->get_const_float(cid));
...
int_val get_op_function<int_val>(const instr_val& val, pcounter& m, state& state)
{
   if(val_is_int(val)) {
      ...
      return tuple->get_int(field);
		...
		return vm::All->get_const_int(cid);
   } else
...
node_val get_op_function<node_val>(const instr_val& val, pcounter& m, state& state)
{
   if(val_is_host(val))
      ...
      return tuple->get_node(field);
		...
		return vm::All->get_const_node(cid);
   } else
...
cons* get_op_function<cons*>(const instr_val& val, pcounter& m, state& state)
{
   if(val_is_reg(val))
      ...
      return cons::null_list();
		...
		return vm::All->get_const_cons(cid);
	} else
...
struct1* get_op_function<struct1*>(const instr_val& val, pcounter& m, state& state)
{
   if(val_is_reg(val))
      ...
      return tuple->get_struct(field);
		...
		return vm::All->get_const_struct(cid);
	} else
...
rstring::ptr get_op_function<rstring::ptr>(const instr_val& val, pcounter& m, state& state)
{
	if(val_is_reg(val))
		...
		return vm::All->get_const_string(cid);
	} else
...
execute_cons(pcounter pc, state& state)
{
   ...
   list_type *ltype((list_type*)vm::All->PROGRAM->get_type(cons_type(pc)));
...
{
   if(state.node->get_id() > vm::All->DATABASE->static_max_id())
...
execute_colocated(pcounter pc, state& state)
{
   ...
   //state.set_bool(dest, vm::All->MACHINE->same_place(n1, n2));
...
execute_delete(const pcounter pc, state& state)
{
   ...
   const predicate *pred(vm::All->PROGRAM->get_predicate(id));
...
execute_calle(pcounter pc, state& state)
{
   ...
   switch(ret_type->get_type()) {
      ...
      case FIELD_STRUCT: {
         runtime::struct1 *s(FIELD_STRUCT(ret));
...
execute_new_node(const pcounter& pc, state& state)
{
   ...
   node *new_node(vm::All->DATABASE->create_node());
...
execute_new_axioms(pcounter pc, state& state)
{
   ...
   while(pc < end) {
      ...
      predicate *pred(vm::All->PROGRAM->get_predicate(pid));
...
execute_make_struct(pcounter pc, state& state)
{
   ...
   struct_type *st((struct_type*)vm::All->PROGRAM->get_type(make_struct_type(pc)));
...
execute_mvregconst(pcounter& pc, state& state)
{
   ...
   vm::All->set_const(id, state.get_reg(pcounter_reg(pc + instr_size)));
   if(reference_type(vm::All->PROGRAM->get_const_type(id)->get_type())) {
      runtime::ref_base *p((runtime::ref_base*)vm::All->get_const(id).ptr_field);
...
execute_mvconstfield(pcounter& pc, state& state)
{
   ...
   tuple->set_field(field, vm::All->get_const(id));
   assert(!reference_type(tuple->get_field_type(field)->get_type()));
}
...
execute_mvconstreg(pcounter& pc, state& state)
{
   ...
   state.set_reg(reg, vm::All->get_const(id));
}
...
execute_mvworldfield(pcounter& pc, state& state)
{
   ...
   tuple->set_int(field, vm::All->DATABASE->nodes_total);
}
...
execute_mvworldreg(pcounter& pc, state& state)
{
   ...
   state.set_int(reg, vm::All->DATABASE->nodes_total);
}
...
static inline return_type
execute(pcounter pc, state& state, const reg_num reg, vm::tuple *tpl)
{
	if(tpl != NULL) {
...
eval_loop:
      ...
      switch(fetch(pc)) {
               ...
               if(match_found == state.match_store.end()) {
                  ...
                  if(!iter_options_const(iter_options(pc))) {
                     for(size_t i(0); i < mobj->variable_matches.size(); ++i) {
                        ...
                        vm::tuple *tpl(state.get_tuple(tmp.reg));
         ...
         case CALLF_INSTR: {
              ...
              function *fun(vm::All->PROGRAM->get_function(id));
