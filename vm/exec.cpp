...
{
   tuple *tuple(vm::tuple::create(vm::All->PROGRAM->get_predicate(alloc_predicate(pc))));
...
#endif
      vm::All->MACHINE->route_delay(state.sched, state.node, tuple, state.count, state.depth, send_delay_time(pc));
   } else {
#ifdef DEBUG_SENDS
...
{
   struct_type *st((struct_type*)vm::All->PROGRAM->get_type(make_structr_type(pc)));
...
execute_mvregconst(pcounter& pc, state& state)
{
   ...
   if(reference_type(vm::All->PROGRAM->get_const_type(id)->get_type()))
      do_increment_runtime(vm::All->get_const(id));
}
