...
#ip addressses of the nodes in fawn
ONE="192.168.100.208"
...
run_timer()
{
	...
	local NUM=1
    #vary the inner loop size to compute data
    #the outer loop is made to be 500, however can be varied as well
	for l in 1000000 2000000 3000000 4000000 5000000 6000000 7000000 8000000 9000000 10000000; do
		...
		for number in $ONE $TWO $THREE $FOUR $FIVE $SIX $SEVEN $EIGHT; do
            #vary whether to rdvs or not
			for rdvs in 0 1; do
                #vary number of proccesses
			    for i in 1 2 3 4; do
					if  [ "$number" = $ONE ]; then
        	            NUM=1
